...
class AppStore extends Component {
  defaultApps = appsList.filter(eachApp => eachApp.category === 'SOCIAL')

  state = {apps: this.defaultApps, value: '', activeTabId: tabsList[0].tabId}

  updateApps = (tabId, value) => {
    this.setState({
      apps: appsList.filter(eachApp => eachApp.category === tabId),
    })
  ...
  render() {
    const {apps, value, activeTabId} = this.state

    const searchResults = apps.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(value.toLowerCase()),
    )
